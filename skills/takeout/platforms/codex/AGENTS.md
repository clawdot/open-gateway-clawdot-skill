...
1. `--shop-id shop_xxx` → 分类概览（含 item_id/价格）
2. `--shop-id shop_xxx --category "热饮"` → 分类下所有商品
3. `--shop-id shop_xxx --item-id item_xxx` → 商品详情，含 `sku_options[]`（带 `sku_id`）、`ingredient_options[]`（带 `option_id` + `selected_by_default`）
