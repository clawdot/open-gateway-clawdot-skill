...
## [2.4.0] - 2026-09-20 — 对齐接口文档 v4.5

对照官方《API/MCP 接口说明文档 v4.5》全量核对（线上 `tools/list` 24 个工具的入参 schema
逐项验证，与文档一致）。下单主链路契约未变，本次补的是**三类缺口**：agent 被要求说、
却拿不到数据的字段；能看见、却没有能力操作的字段；以及网关新能力带来的提速。

### Fixed

- **月售 `tip_texts` 此前被裁掉**：GUIDE 一直要求推荐理由写月售，而店铺级
  `monthly_sales_text` 网关**恒为 null**（文档 §8.1 明写，线上 5/5 实测），真正有值的是
  **商品级** `tip_texts`（实测某店 88 件中 80 件有值）——它却被 `_item_overview` /
  `build_item_detail` 的白名单丢弃。等于要求 agent 说一个它拿不到的数字。现已透出，
  并把 GUIDE 的月售规则改口径为商品级、无 `tip_texts` 时一个销量数字都不许出现。
  同时移除恒 null 的店铺级 `monthly_sales` 键（留着只会把规则指向空字段）。
- **促销标 `promo_labels` 此前被裁掉**：它不只是营销装饰——**「单点不送」这类
  可下单性约束只由这个文案承载**（实测某奶茶店 88 件中 21 件带标、全为「单点不送」）。
  丢掉它，agent 会推荐一个单点下不了单的商品，直到 preview 才撞墙。现已透出（含
  `sub_texts` 限制条件），GUIDE 补对应规则。
- **解绑返回值被整个丢弃**：`revoke_user_bind` 的 `revoked_scopes`（doc §6.5）从未被接收。
  跨能力互通的凭证一次解绑会**连带撤销跑腿**，而用户只被告知"外卖解绑完成"。现已透出，
  多能力时在 message 里点名提示。

### Changed

- **`recommend` 改用网关侧招牌菜，不再并行拉 N 份整菜单**（doc v4.2 `with_recommendations`）：
  平台按月售 + 商家热销/招牌标注直接给出每店 1~2 个招牌菜（自带可下单 `item_id` 与
  `needs_spec_selection`）。线上实测 **6.38s → 1.81s、输出 7295 → 2099 字节**，且素材比
  "每分类取前两件"更贴近真实热销。网关没给招牌菜的店**回落**到原并行拉菜单路径，
  导购素材不会比改动前少；`menus` 键仅在发生回落时出现。

### Added

- **`get_shop_info` 子命令**（doc §8.7）：营业时间 / 是否在营业 / 评分 / 地址，不含菜单，
  ~0.6s。此前用户问"这家几点关门"只能去拉整份菜单。`is_open_now` 为 null 时附提示，
  防止 agent 把"营业时间没解析出来"说成"已打烊"。
- **`get_item_description` 子命令**（doc §8.6）：商家填写的商品说明卡（原料 / 份量 /
  口味 / 做法 / 是否含咖啡因），用于忌口、过敏、份量类提问。条目名由商家自定义、
  不是固定字段，GUIDE 要求原样展示；`details` 为空时附提示，禁止靠商品名推测原料。
- **`preview_order --coupon-ids`**（doc §10.1）：此前 agent 能在 preview 返回里**看到**
  `available_coupons`，却没有任何参数能换券。三态严格区分——不传＝平台自动选最优券
  （默认，不变）、`none`＝本单不用券、逗号分隔 id＝改用指定券。

### Docs（GUIDE 规则，无对应代码改动）

- **`search_match_level` 补规则**：该字段 v2.3.0 已透出，但 GUIDE 里零条规则——数据到了、
  agent 不知道怎么用等于没给。现明确：`related` ＝这个地址点不到该品牌，必须如实说
  "附近没有 X"，禁止把同类推荐店当成用户要的那家。
- **`get_item_options` 语义变更**（doc v3.0）：返回列表已按当前选择**收敛**，列出来的都能选；
  推论是改了上层选择（换咖啡豆/杯型）**必须重调一次**，拿旧列表继续选会选到不兼容项。
- **规格粗细并存**：同一商品可能同时有"大杯(473ml)/热"与"少冰"两种粒度的规格，各自独立
  可下单，看 `specs` 判断它定了哪些维度，别硬凑矩阵。
- **查单新字段**：`cancel_reason`（原样转述，禁止自编原因与枚举判断）、
  `delivery.rider_name` / `rider_phone`（均 null ＝还没派到骑手，不是查询失败）、
  `detail_url`（可发给用户；每次返回可能不同，不许当订单标识前后比对）。

### 已知未收口（诚实记录，勿当已修）

- **45 项大清单仍会编造选项名**（`large-option-list-no-truncation` 场景通过率约 1/3）。
  排查结论**推翻了"本次改动造成回归"的初判**：改动前 mock 的 `recommend` 直接返回
  **整份菜单（含全部 45 个选项名）**，而真 CLI 的 `recommend` 走
  `build_menu_overview(compact=True)`——**只给 has_skus / has_ingredients 两个布尔、
  一个选项名都不给**。那份"改动前更高的通过率"是 mock 不忠实喂出来的额外曝光，
  线上从不存在。本次 mock 与真 CLI 对齐后，暴露的是模型**本来就有**的编造倾向。
  已补两道防线：① 概览新增 `spec_fetch_hint`，点明概览不含规格、必须先取真实选项；
  ② GUIDE 大清单规则点名最常见破口——**把一个组的命名规律套到另一个组**
  （看「时蔬」都是 `A+B`，就给「肉肉」编出 `肥牛+羊肉卷`）。通过率从 0/3 回到 1/3，
  **未恢复到可接受水平，留作未决项**。
  遗留：mock 的 `get_shop_menu --shop-id` 仍返回全量菜单（真 CLI 返回裁剪概览），
  这条不忠实还在，修它会动到多条存量场景的基线，单独一轮处理。

  ⚠️ 本轮**新引入**的两处 mock 不忠实已在合并前修掉（独立验收抓出）：`recommend` 原本
  从不产 `menus` 键（回落分支在 eval 里根本走不到）、`search_match_level` 只在
  `search_shops` 分支计算（而 GUIDE 把品牌名路由到 `recommend`，"搜瑞幸"场景拿不到该字段，
  比真 CLI 更严苛地藏了字段）。既然"已知未收口"那节的全部论证都建立在"mock 不忠实会喂出
  假的通过率"上，新增的不忠实同样不能留。修 mock 时**顺带暴露出我写错的一条 GUIDE 规则**：
  品类词（奶茶/咖啡）在真网关上也返回 `related`，只按字段判会让 agent 说"附近没有奶茶"——
  文档把该警告限定在**搜店铺名**场景，规则已补上这个限定并加了 eval 场景钉住。

### 未纳入（实测后判负，记录理由避免重复评估）

- **`grouped=true`**（加料按组返回）：线上实测传输 -15.2%，但 agent 看到的是 CLI 裁剪后的
  结构、**模型侧 token 不变**；而切换要重写 GUIDE 的规格展示规则、eval 的
  `no_invented_options` 检查器与 mock。风险大于收益。
- **`full_menu=true`**（菜单大纲）：实测 445KB→70.8KB、3.26s→1.27s，但大纲不含规格、
  `--item-id` 将无法从缓存菜单取到 sku/加料，需改为每次回查 get_item_options——
  总耗时大致抵消，却新增一条失败路径。
- **`chain_brands_only`**：判据是"是不是连锁"而非"好不好"，会一并筛掉口碑单店（网关侧
  DECISIONS D-E 已明确这是代理指标），C 端点单场景是负体验。
- **`exclude_required_selection`**：网关侧实测奶茶店留存 38%、茶百道 13~17%，且会滤掉
  麻辣烫这类本就有必选组的正常品类——本 skill 有完整的必选组处置流程，不需要绕开。
- **`batch_get_item_options`**（v4.5 新）：跨店批量比规格，而本 skill 是"先定店再选菜"的
  单店闭环，当前无调用场景。
- `lang` / `include_chinese`（中文场景无需）、`quote_cart`（preview 自带算价）、
  `list_orders` / 免密签约 / `delete_address` / `get_homepage_url`（超出点单主链路）。
...
- **`API_KEY_INVALID` 错误文案补入注册页 URL**：此前该分支只说"按注册页引导用户"却不给
  地址——在"禁止编造链接"的输出纪律下，助手会正确地拒绝编 URL，但也因此无处引导、
  对话卡死。现在 stderr 直接携带注册页地址（`CLAWDOT_SETUP_URL` 可覆盖，默认
  控制台登录页），助手原样转发即可。

## [2.2.1] - 2026-07-24 — 授权引导忠实性

### Fixed

- **未绑定 ≠ key 问题**：`USER_NOT_BOUND_NEEDS_SMS`（还没绑定用户）时，禁止说
  "API_KEY 不对/失效/重新申请"、禁止发任何注册链接，直接进绑定引导（选 H5 / 短信验证码）。
- **注册链接只能原样转发**：`API_KEY_MISSING` 引导中的链接必须逐字抄脚本 stderr 给出的
  那一个，禁止输出任何其他域名。
- 反面教材新增对应真实案例。
