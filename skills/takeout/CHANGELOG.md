...
## [2.1.2] - 2026-07-22 — 输出规范：规格清单格式

### Changed

- **规格/选项展示统一为清单形态**（Step 4.5 新增「规格清单格式」模板 + 示例）：规格行带
  各自价格用 `|` 分隔，每个属性组（温度/甜度/加料…）各占一行、组内用 `/` 分隔、默认项标
  `（推荐）`排首位、加价项标 `+¥X`；禁止散文式罗列与展示内部 id。店铺必选组
  （`required_groups`）的候选项沿用同一格式。选项更易扫读、比价更直观。话术层更新，无代码改动。
