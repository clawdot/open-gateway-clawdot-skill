...
联系人：`--from-name/--from-phone`、`--to-name/--to-phone`。缺省优先级：
**显式入参 > 地址簿存的 > 发件端兜底下单人本号 / 收件端问一次**。
⚠️ **收件端绝不套下单人本号**——否则骑手会打给下单人而不是收件人。走 `--to-id` 时地址簿
存了联系人电话就自动带上；都没有则网关回 `CONTACT_REQUIRED`，按提示问一次并顺手存进地址簿。
