...
所有操作通过 `python3 scripts/errand.py <command> [--phone <手机号>]` 调用（子命令 1:1 对应网关 MCP tool 名，内部映射到 `errand_*` 工具）：
...
| 情形 | 业务调用方式 | 凭证来源 |
|------|------------|---------|
| 单用户（默认） | 不传 `--phone` | 共享缓存（`~/.clawdot/errand-credentials.json`，按 API_KEY+手机号键控）中唯一已绑用户；也可用 `CONSENT_GRANT_ID` 环境变量预注入（只读） |
| 多用户 | 传 `--phone <11 位>` | 各用户各自绑定，按手机号存共享缓存 |

唯一必需注入的是 `API_KEY`（agent 身份，须已开通 errand 能力）。一个 api_key 可服务多个用户，**一个 cg = 一个用户**。绑定步骤需 `--phone`；绑定成功写入共享缓存后单用户业务调用无需再带 `--phone`。`API_KEY` 没配时返回 `RECOVERY[API_KEY_MISSING]`。**open-gateway 无 admin 静默绑定**——每个用户必须本人走一次短信授权。
...
- 按 RECOVERY 提示选下一步；下单/加小费/取消先经用户确认（取消先报违约金）
